...
app.get('/devices/:username', function(req,res,next){
	db.getConnection(function(err, connection) {
		if (err) {
			...
			connection.query('select * from (device_status d join device_instance i) join device_capability_template WHERE property_id = capability_id AND belongTo = \''+ req.params.username +'\'',function(err,rows,fields){
				if (err) {
					console.esrror(err);
					res.statusCode = 500;
					res.send({
